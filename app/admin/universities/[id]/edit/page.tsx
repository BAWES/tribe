...
export default function EditUniversityPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    const fetchUniversity = async () => {
      setIsLoading(true)
      try {
        // In a real app, this would be an API call to get the university
        // For now, we'll use mock data based on the ID
        await new Promise((resolve) => setTimeout(resolve, 500))

        // Mock data for university with ID 1
        if (params.id === "1") {
          form.reset({
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    try {
      // In a real app, this would be an API call to update the university
      console.log(values)
  ...
  if (isLoading) {
    return (
      <div className="container px-4 md:px-6 py-10 flex items-center justify-center min-h-[60vh]">
  ...
  return (
    <div className="container px-4 md:px-6 py-10">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        <h1 className="text-3xl font-bold">Edit University</h1>
      </div>

      <Card className="bg-white/5 border-white/10">
        <CardHeader>
          <CardTitle>University Information</CardTitle>
          <CardDescription className="text-zinc-400">Update the details for this university</CardDescription>
        </CardHeader>
        <Form {...form}>
